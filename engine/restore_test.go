...
func TestCollectorRestoreSeedsCounters(t *testing.T) {
	...
	collector.Restore([]TrafficSnapshot{
		{RuleID: "r1", UploadBytes: 100, DownloadBytes: 200, UDPSessionRejected: 3, UDPPacketsDropped: 5},
		{RuleID: "r2", UploadBytes: 50, DownloadBytes: 7},
	})
	snap := collector.Snapshot("r1")
	if snap.UploadBytes != 100 || snap.DownloadBytes != 200 || snap.UDPSessionRejected != 3 || snap.UDPPacketsDropped != 5 {
